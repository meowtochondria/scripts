...
CPANEL_API_USER=$($KEEPASSXC_CLI show --quiet --attributes username $KEEPASSXC_DB '/Home/Namecheap CPanel API' <<< $KEEPASS)
...
for domain in devghai.com ghai.co cloud.devghai.com git.devghai.com src.devghai.com; do
    curl -H "Content-Type: text/plain" -H "Authorization: cpanel ${CPANEL_API_USER}:${CPANEL_API_TOKEN}" "https://premium59.web-hosting.com:2083/execute/SSL/install_ssl" --data "domain=$domain" --data-urlencode "cert@certificates/devghai.com.crt" --data-urlencode "key@certificates/devghai.com.key" | jq .
done
